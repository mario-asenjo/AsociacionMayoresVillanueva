  ...
  fun parse(userText: String): AgentIntent {
    ...
    if (containsAny(text, "actividad", "actividades", "rutina", "rutinas")) {
      val scope = when {
        ...
        containsAny(text, "manana", "mañana", "proximas", "próximas", "semana") -> ActivitiesScope.UPCOMING
        containsAny(text, "inscritas", "inscrita", "apuntadas", "apuntada", "mis") -> ActivitiesScope.REGISTERED
